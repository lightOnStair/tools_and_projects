...
el_blockhead_t *el_block_below(el_blockhead_t *block){
  ...
  size_t lowersize = lowerfoot->size;
  el_blockhead_t *lower =
    PTR_MINUS_BYTES(block, lowersize + EL_BLOCK_OVERHEAD);
...
el_blockhead_t *el_find_first_avail(size_t size){
  ...
  el_blockhead_t *end = el_ctl.avail->end;
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){
  ...
  block->size = new_size;
  el_blockfoot_t *foot = PTR_PLUS_BYTES(block, sizeof(el_blockhead_t)+new_size);
  foot->size = new_size;
  el_blockhead_t *head = PTR_PLUS_BYTES(foot,sizeof(el_blockfoot_t));
  head->size = totalsize - new_size - EL_BLOCK_OVERHEAD;
...
void *el_malloc(size_t nbytes){
  ...
  el_blocklist_t *used = el_ctl.used;
  ...
  el_blockhead_t *new= el_split_block(block,nbytes);
  printf("awdawdwaddddddddddddddddddddddddddddddddd");
  printf("%ld",new->size);
  el_remove_block(avail, block);
  el_add_block_front(used, block);

  el_add_block_front(avail, new);
...
void el_free(void *ptr){
  el_blockhead_t *block = ptr;
  if(block==NULL || block->state==EL_USED){
    return;
  }

  el_blockhead_t *higher = el_block_above(block);
  if(higher==NULL || higher->state == EL_USED){
